...
def request_magic_link():
    ...
    # In dev mode without SMTP, pass the magic link URL to the template for quick developer login
    dev_magic_url = None
...
def verify_magic():
    ...
    if not user:
        # First user created in the system becomes Admin automatically
    ...
    next_url = request.args.get("next", "")
    target_redirect = next_url if is_safe_redirect_url(next_url) else url_for("auth.profile")

    response = make_response(redirect(target_redirect))
